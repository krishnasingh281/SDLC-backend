from flask import Blueprint, jsonify, request
from spectree import Response
from app.core.schemas import GenerationRequest, GenerationResponse
from app.core.llm import run_code_generation
from app.core.docs import api as docs, USE_DOCS

# Define the Blueprint with a URL prefix
bp = Blueprint("codegen", __name__, url_prefix="/codegen")

@bp.post("/")
def handle_code_generation():
    """
    Handles the request for PS-08: Code Generation & Efficiency.
    """
    # Use validated data placed in request.context by spectree, or fall back to manual validation
    if USE_DOCS and hasattr(request, 'context') and request.context.json:
        validated_body = request.context.json
    else:
        try:
            validated_body = GenerationRequest.model_validate_json(request.data)
        except Exception as e:
            return jsonify({"msg": f"Invalid request body format: {e}"}), 400
            
    # Call the core LLM function
    resp = run_code_generation(validated_body)
    
    # Return the dumped JSON and HTTP 200 OK status
    return jsonify(resp.model_dump()), 200


if USE_DOCS:
    # Use the correct Spectree Response wrapping pattern
    handle_code_generation = docs.validate(
        json=GenerationRequest,
        resp=Response(HTTP_200=GenerationResponse),
        tags=["coding", "efficiency"]
    )(handle_code_generation)